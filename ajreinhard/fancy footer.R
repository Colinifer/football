...
brand_plot <- function(orig_plot, save_name, asp = 1, base_size = 5, data_home = '', fade_borders = '', axis_rot = F) {
  ...
  logo_file <- readPNG('ajreinhard/statbutler.png')
  
  author_txt <- shadowtextGrob('By Anthony Reinhard', x=unit(0.08 * (base_size_rat_wid), 'npc'), gp=gpar(col='darkblue', fontsize=6), hjust=0)
  data_txt <- shadowtextGrob(data_home, x=unit(1 - (.01 * (base_size_rat_wid)), 'npc'), gp=gpar(col='grey95', fontsize=6), hjust=1)
...
brand_plot_no_save <- function(orig_plot, save_name, asp = 1, base_size = 5, data_home = '', fade_borders = '') {
  ...
  author_txt <- textGrob('By Anthony Reinhard', x=unit(0.08 * (base_size_rat_wid), 'npc'), gp=gpar(col='darkblue', fontsize=6), hjust=0)
  data_txt <- textGrob(data_home, x=unit(1 - (.01 * (base_size_rat_wid)), 'npc'), gp=gpar(col='grey95', fontsize=6), hjust=1)

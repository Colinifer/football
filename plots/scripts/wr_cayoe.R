...
cayoe_filtered <- cayoe %>% 
  filter(targets >= ifelse(summary(targets)[4]>75, 75, summary(targets)[4]))

summary(cayoe_filtered$targets)

cayoe_filtered <- cayoe_filtered %>% 
  filter(targets >= ifelse(summary(targets)[4]>75, 75, summary(targets)[4]))
...
cayoe_filtered %>%
  ...
  tab_header(title = paste('Completed Air Yards Over Expected (CAYOE),', cayoe_filtered$season[1]), 
             subtitle = paste('Through week', my_week, 'MNF', '|', 'Min.', ifelse(round(summary(cayoe_filtered$targets)[4])>75,75,round(summary(cayoe_filtered$targets)[4])),'pass attempts > 0 air yards')) %>% 
  ...
  fmt_number(columns = vars(air_yards, exp_air_yards, exp_completions), decimals = 0, sep_mark = ',') %>% 
  ...
  tab_spanner(label = 'Actual', columns = vars(completions, air_yards, td)) %>% 
  ...
  gtsave(filename = glue("wr_cayoe_{cayoe_filtered$season[1]}.png"), path = "plots/desktop")

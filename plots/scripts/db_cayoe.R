  ...
  left_join(
    readRDS(glue('data/pbp/sportradar/sr_pbp_{current_season}.rds')) %>% 
      select(
        ...
        game_id,
        play_id,
        defPlayers.def_target
      ) %>%
      filter(
        !is.na(defPlayers.def_target)
      )
  )
...
cayoe_filtered %>%
  ...
  tab_header(title = paste('Completed Air Yards Over Expected (CAYOE),', cayoe_filtered$season[1]), 
             subtitle = paste('Through week', my_week, '|', 'Min.', ifelse(round(summary(cayoe_filtered$targets)[4])>75,75,round(summary(cayoe_filtered$targets)[4])),'pass attempts > 0 air yards')) %>% 
  ...
  gtsave(filename = glue("db_cayoe_{cayoe_filtered$season[1]}.png"), path = "plots/desktop")

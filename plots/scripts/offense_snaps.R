p <- player_stats_weekly %>% select(player_id,
                               player_name,
                               recent_team,
                               week,
                               offense_snaps,
  ...
  left_join(roster_df %>% 
            ...
            by = c('player_id')) %>% 
  filter(position %in% c('RB', 'WR', 'TE') & offense_pct >= 0.43) %>% 
  arrange(-snap_pct_diff) %>% 
  ggplot(aes(x = week, y = offense_pct)) + 
  geom_line(aes(group = player_id)) + 
  geom_text_repel(data = player_stats_weekly %>% filter(week > 1), aes(label = player_name)) +

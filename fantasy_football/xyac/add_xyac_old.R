add_xyac_old <- function(pbp) {
  ...
  if (!nrow(passes) == 0) {
    ...
    xyac_model <- NULL
    suppressWarnings(# load the model from github because it is too big for the package
      try(load(
        url(
          "https://github.com/guga31bb/nflfastR-data/blob/master/models/xyac_model.Rdata?raw=true"
        )
      ),
      silent = TRUE))
    if (!is.null(xyac_model)) {
      xyac_vars <-
        stats::predict(xyac_model,
                       as.matrix(passes %>% xyac_model_select())) %>%
        tibble::as_tibble() %>%
        dplyr::rename(prob = "value") %>%
        dplyr::bind_cols(purrr::map_dfr(seq_along(passes$index), function(x) {
          tibble::tibble(
            "yac" = -5:70,
        ...
        dplyr::summarise(
          ...
          xyac_mean_yardage = (
            dplyr::first(.data$original_spot) - dplyr::first(.data$air_yards)
          ) - sum(.data$wt_yardln),
          xyac_median_yardage = max(.data$med),
          xyac_success = sum((.data$ep > .data$original_ep) * .data$prob),
          xyac_fd = sum((
            .data$gain >= .data$original_ydstogo
          ) * .data$prob),
          .groups = "drop_last"
        ) %>%
        dplyr::ungroup()
      ...
      # means xyac_model isn't available
      message(

...
dataset <- pbp_df %>% 
    filter(week >= my_week - 4 & pass_attempt==1 & season_type=='REG' & two_point_attempt==0 & !is.na(receiver_id)) %>% 
    add_xyac_dist %>% 
...
ui <- fluidPage(
    ...
    sidebarPanel(
        ...
        selectInput('posteam', 'Possession Team', choices = dataset$posteam %>% unique() %>% sort(decreasing = FALSE))
    ),
    mainPanel(gt_output("xfantasy_points"))
)

# Define server logic required to draw a histogram
server <- function(input, output) {
    
    output$xfantasy_points <- gt_output(
        # generate bins based on input$bins from ui.R
        expr = dataset %>% 
            filter(posteam = input$posteam) %>% 
            select(
                games,
                receiver,
                posteam,
            ...
            fmt_number(
                columns = vars(yards, exp_yards, exp_catches),
                decimals = 0,
                sep_mark = ','
            ) %>%
            tab_style(style = cell_text(size = 'x-large'),
                      locations = cells_title(groups = 'title')) %>%
            tab_style(style = cell_text(align = 'center', size = 'medium'),
                      locations = cells_body()) %>%
            tab_style(style = cell_text(align = 'left'), locations = cells_body(vars(receiver))) %>%
            tab_spanner(label = 'Actual',
                        columns = vars(catches, yards, td, half_PPR_pts_pg)) %>%
            tab_spanner(
                label = 'Expected',
                columns = vars(exp_catches, exp_yards, exp_td, exp_half_PPR_pts_pg)
            ) %>%

update_cfb_db_mod <-
            ...
            db_connection = NULL)
  {
    ...
    if (!is_installed("DBI") |
        !is_installed("purrr") | (!is_installed("RSQLite") &
    ...
    if (is.null(db_connection)) {
      connection <-
        DBI::dbConnect(RSQLite::SQLite(), glue::glue("{dbdir}/{dbname}"))
    }
    else {
...
build_cfb_db_mod <-
  function(tblname = "cfbfastR_pbp",
    ...
    if (all(rebuild == TRUE)) {
    ...
    } else if (is.numeric(rebuild) &
               all(rebuild %in% valid_seasons$year)) {
      ...
      seasons <-
        valid_seasons %>% dplyr::filter(.data$year %in% rebuild) %>% dplyr::pull("year")

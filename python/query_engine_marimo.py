...
@app.cell
def _(os):
    # For local connections, set SSH_PARAMS to None
    # For remote connections via SSH tunnel, use:
    SSH_PARAMS = {"profile": "moose"}
    # SSH_PARAMS = None
    ...
    if not all(DB_CONNECTION_PARAMS.values()):
        ...
        mo.stop(
            ...
            # Display these parameters to help debug
            output=DB_CONNECTION_PARAMS
        )
    return DB_CONNECTION_PARAMS, SSH_PARAMS


@app.cell
def _(Any, atexit, psycopg2, socket, sql, subprocess, time):
    class QueryEngine:
                ...
                # Handle simple conditions
                    ...
                    # Handle IN/NOT IN with lists
                        ...
                        if not isinstance(value, (list, tuple)):
                            raise ValueError(
                                f"{op.upper()} operator requires a list value",
                            )
...
@app.cell
def _(DB_CONNECTION_PARAMS, QueryEngine, SSH_PARAMS):
...
@app.cell
def _(DB_CONNECTION_PARAMS, QueryEngine, SSH_PARAMS):
    # Cell 3: Player Stats Fetching Skill
    import polars as pl

    def fetch_player_receiving_stats(
        season: int,
        team: str,
        player_name_filter: str | None = None,
        limit: int | None = None,
    ) -> pl.DataFrame:
        """
        Fetches receiving statistics for players based on season and team,
        with an optional player name filter.

        Args:
            season (int): The NFL season to query.
            team (str): The team abbreviation (e.g., "PHI").
            player_name_filter (str | None): Optional partial name to filter players.
            limit (int | None): Optional limit to the number of rows returned.

        Returns:
            pl.DataFrame: A Polars DataFrame with the receiving statistics.
        """
        filters = {
            "logic": "and",
            "conditions": [
                ...
                {"column": "season", "operator": "=", "value": season},
                {"column": "posteam", "operator": "=", "value": team},
            ],
        }
    ...
    # This cell just defines the function, it doesn't execute it.
    # The function will be available for use in downstream cells.
    return (fetch_player_receiving_stats,)


@app.cell
def _(fetch_player_receiving_stats):
    fetch_player_receiving_stats(
        season=2025,
        team="PHI",
        limit=1000,
    )
...
@app.cell
def _(alt, fetch_player_receiving_stats):
    ...
    _chart = (
        ...
        .mark_bar()
        .encode(
            x=alt.X(field='receiving_yards', type='quantitative', aggregate='sum'),
            y=alt.Y(field='receiver_player_name', type='nominal', sort='descending'),
            tooltip=[
                alt.Tooltip(field='receiver_player_name'),
                alt.Tooltip(field='receiving_yards', aggregate='sum', format=',.2f')
            ]
        )
        .properties(
            height=300,
